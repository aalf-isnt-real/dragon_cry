---
{"dg-publish":true,"permalink":"/character-creation/ability-skills-and-proficiency/abilities/"}
---

Maximum of +6
when rolling Straight Ability, do (Amount of d6s = to modifier) + modifier (min 1)
ex: if rolling a +4 Intuition, roll 4d6 + 4

Influences your [[Character Creation/Ability, Skills & Proficiency/Skills\|Skills]], meet prerequisites for Feats and Manoeuvres

## Physical
STRENGTH
FORTITUDE
AGILITY

## Mental
KNOWLEDGE
INSIGHT
INTUITION / INSTINCT

## Social
PRESENCE
CHARM
ARTISTRY

## SAVES
increases per Class level, can be added to instead of Abilities on an ASI

REFLEX
VOLITION
FORTITUDE



# Calculating stats

## Starting Array:

| STR | FORT | AGI |     | KNO | INS | INT |     | PRE | CHA | ART |
| --- | ---- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| -3  | -3   | -3  |     | -3  | -3  | -3  |     | -3  | -3  | -3  |

### Point Buy
40 points total to increase stats
stat increase above 3 costs 2 points, cannot go above 5

## Standard Array

| 5   | 4   | 3   | 2   | 2   | 2   | -2  | -2  | -3  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |

## Stat Increases
Ability Increases give 3 stat points to allocate, can be put in saves

Increasing an ability or save from 5 to 6 costs 2 Stat Points